...
const GradientCarousel: React.FC<GradientCarouselProps> = ({
  ...
  const renderBackground = useCallback(
    function renderBg() {
      ...
      grad1.addColorStop(1, "rgba(0,11,56,0)");
      ctx.fillStyle = grad1;
      ctx.fillRect(0, 0, w, h);
      ...
      grad2.addColorStop(1, "rgba(0,11,56,0)");
      ctx.fillStyle = grad2;
      ctx.fillRect(0, 0, w, h);
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative h-full w-full overflow-hidden bg-teget",
    ...
    >
      {/* Loading indicator */}
      {isLoading && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-teget">
          <div className="h-9 w-9 animate-spin rounded-full border-[3px] border-white/15 border-t-zelena" />
        </div>
      )}
        ...
        {stableImages.map((src, i) => (
          ...
          >
            <img
              src={src}
              alt={`Matchday ${i + 1}`}
              className="pointer-events-none h-full w-full select-none rounded-[1.75rem] border border-white/10 object-cover shadow-2xl"
              draggable={false}
