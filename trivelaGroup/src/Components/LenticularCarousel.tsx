...
/* Adaptirano iz react-bits LenticularCarousel: bez "use client", cn iz
   ../lib/cn, i inline chevron umesto lucide-react. */
...
const Deck = ({
  ...
  useFrame((_, raw) => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const rawRel = i - glide.current;
      const rel = loop ? nearest(rawRel, count) : rawRel;
    ...
    <group>
      {items.map((item, index) => (
        <mesh
          key={`${item.src}-${index}`}
          ref={(node) => {
            cards.current[index] = node as THREE.Mesh | null;
          }}
          geometry={plane}
...
const LenticularCarousel: React.FC<LenticularCarouselProps> = ({
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const dt = Math.max(4, event.timeStamp - grab.at);
    grab.vel = grab.vel * 0.6 + ((event.clientX - grab.last) / dt) * 0.4;
